...
- Install & setup docker (cf https://faun.pub/ros2-humble-gui-docker-container-a-step-by-step-guide-c541b73fe141) :
...
=> Restart the system to apply the group changes
...
git submodule foreach -q --recursive 'git switch $(git config -f $toplevel/.gitmodules submodule.$name.branch || echo master)' # switch on branches defined by .gitmodules
git submodule foreach -q --recursive 'git checkout tags/$(git config -f $toplevel/.gitmodules submodule.$name.tag || echo master)' # checkout to tags defined by .gitmodules
cd multirobots_ws/src/PX4-Autopilot
git submodule update --init --recursive # Init PX4-Autopilot package's submodules
